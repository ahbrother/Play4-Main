...
updateSeats(newCount) {
  ...
  if (diff > 0) {
    ...
    for (let i = 0; i < diff; i++) {
      this.seats.push({
        pos: createVector(this.pos.x + random(-20, 20), this.pos.y + random(-20, 20)),
        vel: createVector(random(-1, 1), random(-1, 1))
      });
    }
  } else if (diff < 0) {
  ...
  update() {
    ...
    for (const seat of this.seats) {
      ...
      seat.pos.add(seat.vel);
        ...
        // constrain to country bounds
  const isRight = this.side === "right";
 // const xMin = isRight ? windowWidth / 2  : 0;
 // const xMax = isRight ? windowWidth : windowWidth / 2 - 10;
...
if(isRight){
  translate(windowWidth/2,0)}
    ...
    */
    for (const seat of this.seats){
      fill(color);
      rect(seat.pos.x, seat.pos.y, 15, 15);
    }
